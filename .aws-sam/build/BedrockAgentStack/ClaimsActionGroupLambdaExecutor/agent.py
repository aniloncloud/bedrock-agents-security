...
import boto3
import os
# from shared import verifyJWT_getUserInfo
import logging
...
def verifyAccess(sessionAttributes, action_id):
    ...
    if not sessionAttributes:
        ...
        if not authorization_header:
            ...
            user_info = verifyJWT_getUserInfo(authorization_header, os.environ["AWS_REGION"], os.environ["USER_POOL_ID"])
            if not user_info:
...
def handle_is_authorized(user_info, action_id):
    ...
    resource_type = operation_mapping.get(api_operation, action_id)

     # 1. Construct the authorization request   
    construct_authz_request_from_token(user_info, action_id, resource_type)
